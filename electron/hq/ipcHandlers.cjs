...
const registerIpcHandlers = ({
    ...
    ipcMain.handle('stones:media-stage-file-discard', async (_event, fileId) => requireMediaQueue().discardStagedFile(ensureStringId(fileId, 'fileId')));
    ...
    ipcMain.handle('stones:complete-photo-apply-workflow-staging', async (_event, workflowId) => requireMediaWorkflowManager().completeWorkflowStaging(ensureStringId(workflowId, 'workflowId')));
