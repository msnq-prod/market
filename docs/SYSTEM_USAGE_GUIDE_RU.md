...
- `Наличие` — плотная таблица остатков с поиском по названию, локации, кодам, `serial_number` и `temp_id`;
...
- из карточки партии доступны быстрые переходы в `Photo Tool`, `Video Tool` и `QR Print`; в браузере media tools показывают заглушку со скачиванием `ZAGARAMI HQ`.
...
- V2 run хранит item-level render/upload state и обновляет `item_video_url` после загрузки item-файла;
