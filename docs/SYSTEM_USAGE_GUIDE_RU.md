...
Экран `/admin/warehouse` — древовидный центр навигации по складу и заказам на сбор.

Возможности:
- видеть дерево `Location -> Product -> Batch -> Item`;
- получать краткую сводку по каждой локации и товару;
- переключать товар между режимами `Партии` и `Все товары`;
- раскрывать партии и смотреть `Item` сеткой;
- видеть fallback-группы `Без локации` и `Без товара` для legacy-данных;
- открывать полный modal `Item` и редактировать рабочие поля;
- видеть `CollectionRequest` как вторичный блок контекста.
...
- при переводе в `IN_STOCK` новые `Item` автоматически получают статус `STOCK_HQ`.
...
- удалённые фрагменты не попадают в export-session и могут оставлять пропуски по таймкоду исходника;
...
Именно после `Allocation` экземпляр попадает в публичный доступный остаток.
...
8. HQ переводит партию в `IN_STOCK`, а новые `Item` попадают в `STOCK_HQ`.
9. HQ распределяет `Item` через `/admin/allocation` в `STOCK_ONLINE` или `ON_CONSIGNMENT`.
10. Публичная карточка шаблона начинает показывать актуальный остаток только после перехода в `STOCK_ONLINE`.
11. Покупатель получает QR и открывает паспорт своего конкретного `Item`.
