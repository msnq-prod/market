# Полное руководство по использованию ZAGARAMI

Актуально для текущего репозитория в директории `/Users/nikitamysnik/Desktop/progs/stones`.

Публичный бренд системы — ZAGARAMI. В репозитории, env-переменных и части служебных идентификаторов сохраняется внутреннее имя `stones`.

## 1. Назначение системы

ZAGARAMI — это система для:

- публичной витрины уникальных камней;
- работы HQ с каталогом, партиями, складом и пользователями;
- работы франчайзи с задачами на сбор;
- публикации цифрового паспорта конкретного камня;
- базового учета финансов партнера.

Система покрывает полный путь:

1. HQ создает каталог и задачу на сбор.
2. Франчайзи принимает задачу и создает партию.
3. HQ получает и принимает партию целиком, готовит фото/видео и завершает партию.
4. HQ распределяет готовые Item в онлайн-продажу.
5. Покупатель видит товар, отправляет заявку и позже может открыть цифровой паспорт.

## 2. Требования к окружению

- Node.js 22+
- npm 10+
- MySQL 8+

## 3. Быстрый локальный запуск

### 3.1 Установка

```bash
npm install
```

### 3.2 Настройка `.env`

Минимальный пример:

```env
PORT=3001
CLIENT_URL=http://localhost:5173
DATABASE_URL="mysql://stones:stones@localhost:3307/stones?connection_limit=20&pool_timeout=30"
ACCESS_TOKEN_SECRET="change_me_access"
REFRESH_TOKEN_SECRET="change_me_refresh"
TELEGRAM_TOKEN_ENCRYPTION_KEY="change_me_telegram_crypto"
VITE_HOST=127.0.0.1
VITE_PORT=5173
VITE_API_TARGET=http://127.0.0.1:3001
# VIDEO_PIPELINE_DIAGNOSTICS=1
# VIDEO_PROCESSOR_POLL_MS=3000
# VIDEO_PROCESSOR_WORKERS=1
# VIDEO_JOB_CONCURRENCY=2
# VIDEO_JOB_FFMPEG_THREADS=1
# TELEGRAM_WORKER_POLL_MS=5000
# TELEGRAM_WORKER_RETRY_BASE_MS=5000
# TELEGRAM_WORKER_MAX_ATTEMPTS=5
```

### 3.3 Подготовка базы

```bash
npm run db:migrate
npm run db:seed:languages
npm run db:seed
```

### 3.4 Запуск

```bash
npm run dev
```

После запуска:

- frontend: `http://localhost:5173`
- backend: `http://localhost:3001`

## 4. Полезные команды

```bash
npm run lint
npm run build
npm run test:e2e
npm run test:e2e:headed
npm run server
npm run telegram-worker
```

Для изолированного e2e-режима:

```bash
npm run dev:e2e
```

## 5. Роли и доступ

- `ADMIN` — полный HQ-контур
- `MANAGER` — HQ-операции без очереди заказов сайта
- `SALES_MANAGER` — sales-кабинет: `/admin/orders`, `/admin/clients`, `/admin/inventory`, `/admin/sales-history`
- `FRANCHISEE` — только `/partner/*`
- `USER` — публичная витрина и история собственных заказов

## 6. Тестовые аккаунты

После `npm run db:seed` доступны:

- `admin@stones.com` / `admin123`
- `manager@stones.com` / `partner123`
- `sales@stones.com` / `partner123`
- `yakutia.partner@stones.com` / `Partner123`
- `ural.partner@stones.com` / `partner123`
- `baltic.partner@stones.com` / `partner123`
- buyer-логины: `anna`, `maxim`, `olga`, `kirill` / `partner123`

Эти пароли предназначены только для локального dev/e2e. Во внешних окружениях вход с ними блокируется, пока пароль не заменен на уникальный.

Подробности: [TEST_CREDENTIALS_AND_TECH_INFO_RU.md](./TEST_CREDENTIALS_AND_TECH_INFO_RU.md)

## 7. Основные маршруты

### Публичная часть

- `/` — витрина
- `/clone/:serialNumber` — цифровой паспорт

### HQ

- `/admin/login`
- `Сегодня`: `/admin`
- `Продажи`: `/admin/orders/new`, `/admin/orders/in-progress`, `/admin/orders/packed`, `/admin/orders/delivery`, `/admin/orders/returns`, `/admin/orders/closed`, `/admin/clients`, `/admin/inventory`, `/admin/sales-history`
- `Товары`: `/admin/acceptance/batches`, `/admin/warehouse/requests`, `/admin/warehouse`, `/admin/allocation`, `/admin/qr`
- `Планета`: `/admin/products/locations`, `/admin/products`, `/admin/products/publication`, `/admin/planet-labels/workspace`, `/admin/clone-content`
- `Система`: `/admin/system/status`, `/admin/users`, `/admin/telegram`, `/admin/settings`
- контекстное обслуживание склада: `/admin/warehouse/maintenance`
- Telegram-режимы: `/admin/telegram/recipients`, `/admin/telegram/events`, `/admin/telegram/chats`, `/admin/telegram/test`
- fullscreen-инструменты: `/admin/photo-tool/:batchId`, `/admin/video-tool/:batchId`, `/admin/qr/print`, `/admin/planet-labels`

В зоне `Товары` ровно пять вкладок: `Партии / Заявки на сбор / Склад HQ / Распределение / QR-печать`. `Обслуживание` открывается из склада контекстно и не является отдельной вкладкой.

Старые входы `/admin/orders`, `/admin/acceptance`, `/admin/locations`, `/admin/warehouse/items`, `/admin/settings/files`, `/admin/telegram-bots` и `/admin/media*` сохранены только как совместимые перенаправления. Они не создают дополнительные пункты меню.

### Франчайзи

- `/partner/login`
- `/partner/dashboard`
- `/partner/batches`
- `/partner/batches/new`
- `/partner/finance`

## 8. Публичная витрина

### Что видит покупатель

- глобус с локациями;
- подписи локаций используют сохраненную desktop/mobile-раскладку из HQ;
- карточки товаров;
- остаток по доступным item;
- кнопки внешних площадок, если заполнены ссылки;
- корзину и личный кабинет.

### Как работает покупка

1. Покупатель регистрируется или входит.
2. Добавляет товары в корзину.
3. Заполняет адрес, телефон, email и комментарий.
4. Нажимает кнопку оформления.

Результат:

- создается `Order` в статусе `NEW`.

Важно:

- это не реальная оплата;
- Telegram login для buyer-аккаунта пока не подключен;
- Telegram-боты уведомлений для HQ и партнерских событий настраиваются отдельно в admin-панели.

## 9. Работа HQ

### Sales-кабинет

`ADMIN` и `SALES_MANAGER` работают в зоне `Продажи`:

- `Новые / В работе / Упакованы / Доставка / Возвраты / Закрытые` — отдельные этапы заказа;
- `Клиенты` — клиентская база и история заказов;
- `Наличие` — таблица онлайн-остатков по товару и локации;
- `История` — завершенные продажи и возвраты.

На странице `Наличие` доступны фильтры и сортировка по существующим данным. Общая таблица не выводит Item отдельными строками; конкретная позиция появляется только в detail или по точному поиску. Ручное резервирование, создание заказов и массовые операции на этой странице не выполняются.

В desktop-версии `Status Center` содержит диагностику `Проверка создания партии`: оператор выбирает папку с 10 фото и 1 видео, после чего система создает тестовую партию, загружает media, проверяет QR и публичный паспорт.

## 9.1 Сегодня

Показывает один короткий список текущих задач:

- партии в пути;
- принятые партии, требующие продолжения;
- складской остаток HQ;
- неопубликованные карточки.

Каждая строка ведёт в существующий рабочий процесс. Дополнительных вкладок у `Сегодня` нет.

## 9.2 Заказы

Доступно:

- `ADMIN`
- `SALES_MANAGER`

Что можно:

- искать заказы;
- фильтровать по статусу;
- редактировать клиентские поля у незакрытых заказов;
- вести внутреннюю заметку;
- менять статус по допустимым переходам.
- мягко скрывать заказ из интерфейса без физического удаления из БД.

## 9.3 Товары и локации

### Локации

Можно:

- создавать локации;
- редактировать координаты, изображение и базовый перевод;
- редактировать переводы;
- смотреть очередь подписей в HQ shell `/admin/planet-labels/workspace`;
- настраивать горизонтальный и вертикальный вынос подписей на планете в отдельном окне `/admin/planet-labels`;
- мягко скрывать локации из интерфейса. Связанные товарные шаблоны и партии тоже уходят из UI, но остаются в БД.

### Товарные шаблоны

Можно:

- создавать товарные шаблоны;
- редактировать цену, фото, коды и переводы;
- публиковать/снимать с публикации;
- задавать ссылки Wildberries и Ozon;
- создавать задачу на сбор по товару;
- мягко скрывать товарный шаблон из интерфейса без физического удаления.

### 9.3.1 Soft delete и очистка каталога

- Для разовой очистки текущей БД от каталога и связанных операционных сущностей используйте `npm run db:soft-delete:business`.
- Скрипт не удаляет строки физически: проставляет `deleted_at` у локаций, товаров, партий, item, заказов на сбор и заказов покупателей.
- Публичные клоны для скрытых сущностей перестают открываться и возвращают `404`, а не `500`.

## 9.4 Приемка

Рабочий путь:

1. Открыть `Товары → Партии`.
2. Найти партию в `TRANSIT` или `RECEIVED`.
3. Если партия ещё в пути, принять всю партию одним действием.
4. Открыть готовые Photo Tool и Video Tool для подготовки медиа.
5. При необходимости открыть QR-печать.
6. Завершить партию после полной media-готовности.

Приёмка работает на уровне партии: сканирование и ручной проход по Item не используются. Количество строк на странице не зависит от количества Item внутри партии.

Дополнительно:

- в `Склад HQ → Обслуживание` администратор может мягко скрыть партию;
- скрытая партия и связанные item исчезают из интерфейса, но остаются в БД.
- в строке партии видны этапы `Приёмка / Фото / Видео / Завершение`;
- в каждый момент выводится действие текущего этапа;
- Photo Tool и Video Tool в браузере показывают заглушку со скачиванием `ZAGARAMI HQ`.

## 9.5 Photo Tool

Используется HQ для подготовки финальных item-фото перед финализацией партии. Рабочий интерфейс доступен только в desktop-приложении `ZAGARAMI HQ`; браузерный маршрут показывает заглушку со скачиванием DMG.

Сценарий:

1. открыть `/admin/photo-tool/:batchId`
2. загрузить один или несколько файлов изображений
3. при необходимости сменить сортировку, инвертировать список или назначение
4. проверить, что каждая позиция `001..NNN` получила уникальную фотографию
5. сохранить назначения в backend

Особенности:

- Photo Tool работает только для партии в статусе `RECEIVED`;
- у всех item в партии должен быть заполнен `item_seq`;
- можно использовать уже сохраненные фото партии или загрузить новые image-файлы;
- номер под фото соответствует `item_seq`, а не `serial_number`;
- лишние фото допустимы, но без полного покрытия партии сохранить нельзя;
- несохраненный черновик восстанавливается после перезагрузки страницы;
- если другой оператор изменил фото партии, save будет отклонен до обновления страницы;
- в Electron HQ новые локальные файлы при сохранении ставятся в desktop media queue; при плохом соединении задача сохраняется локально и повторяется после восстановления backend;
- backend endpoints остаются доступными для desktop HQ.

## 9.6 Video Tool

Используется HQ для подготовки финальных item-видео. Активный интерфейс — Video Tool v3 в desktop-приложении `ZAGARAMI HQ`.

Сценарий:

1. открыть `/admin/video-tool/:batchId`
2. выбрать первое исходное видео партии с intro
3. дождаться подготовки исходника локальным runtime v3
4. разрезать таймлайн на intro и доступные товарные фрагменты `001..NNN`
5. запустить v3 export
6. дождаться рендера финальных `.mp4`
7. дождаться upload готовых роликов в backend

Особенности:

- Video Tool v3 работает через IPC `window.stones.videoToolV3`;
- подготовка и рендер выполняются локально в HQ Desktop и не зависят от сети;
- сеть нужна только для upload готовых файлов через `/api/video-tool-v3`;
- доступ к Video Tool есть у `ADMIN` и `MANAGER`; `SALES_MANAGER` работает только в sales-разделах;
- backend фиксирует финальные item-файлы и обновляет `Item.item_video_url`;
- failed render/upload можно повторить на уровне item.

## 9.7 QR-печать

HQ-сервис QR PDF работает через:

- `/api/batches/:batchId/qr-pack`
- `/admin/qr`
- `/admin/qr/print?batchId=<ID>&mode=all`
- `/admin/qr/print?batchId=<ID>&mode=selected&ids=<itemId,itemId>`

В QR-пакет попадают только item, которые уже могут иметь публичный паспорт.

Текущий UI-сценарий:

1. открыть `Товары → QR-печать`
2. найти партию в списке
3. выбрать `Печатать все` или `Выбрать QR`
4. в fullscreen-конструкторе выбрать сохраненный пресет или настроить текущий макет
5. при выборочной печати отметить нужные Item
6. настроить размеры этикетки, отступы, QR, шрифты и состав полей
7. сохранить документ через `Сохранить PDF`

По умолчанию этикетки прямоугольные (`Скругление, мм` = `0`), внутренние отступы равны `3 мм` со всех сторон. Порядок полей меняется перетаскиванием карточек настроек, у каждого поля есть отступ сверху и снизу. Блок `Источник данных` можно свернуть, чтобы отдать больше места PDF-превью. Превью рендерится как изображение из того же DOM, что и PDF, поэтому масштабирование экрана не меняет переносы текста. Перед сохранением сервис проверяет, что A4-страницы, размеры этикеток и QR соответствуют заданным параметрам.

Пресеты печати общие для HQ и доступны ролям `ADMIN` и `MANAGER`. В пресет попадает только макет печати: размеры, QR, отступы, скругление, инверсия, порядок/видимость полей, шрифты и интервалы. Товар, партия, выбранные QR и значения `Свое поле` остаются в текущем документе и не сохраняются в пресет.

Поддерживаемые поля этикетки:

- `Название`
- `Локация`
- `Координаты`
- `Время`
- `Серийный номер`
- `Свое поле`

## 9.8 Распределение

Позволяет брать Item из `STOCK_HQ` и переводить их в онлайн-продажу.

Общий список содержит одну строку на партию. Item раскрываются только по явному запросу, когда требуется частичное распределение; для всей партии поштучный список не нужен.

Текущий UI-сценарий:

- `STOCK_HQ -> MARKETPLACE -> STOCK_ONLINE`

## 9.9 Склад HQ

Складской экран позволяет:

- смотреть агрегированные остатки по локациям и товарам;
- видеть количество позиций по существующим складским статусам;
- искать конкретный Item только по точному `serial_number` или `temp_id`;
- открывать найденную карточку Item с media, статусом, QR и clone URL;
- переходить в отдельное обслуживание склада для существующих опасных операций.

Отдельные Item не выводятся в общий список и не раскрываются автоматически.

Ручное изменение item:

- разрешено только `ADMIN`
- только для ограниченного набора полей

## 9.10 Пользователи

Позволяет:

- создавать staff и franchisee-аккаунты;
- видеть роли и balances;
- ограничивать создание ролей по текущему пользователю;
- вручную привязывать `telegram_chat_id` и `telegram_username` к существующему пользователю.

`MANAGER` может создавать:

- `SALES_MANAGER`
- `FRANCHISEE`

`ADMIN` может создавать:

- `ADMIN`
- `MANAGER`
- `SALES_MANAGER`
- `FRANCHISEE`

## 9.11 Telegram

Доступно только `ADMIN`.

Позволяет:

- создавать несколько Telegram-ботов как отдельные вкладки;
- сохранять token бота и проверять его через Telegram API;
- включать/выключать роли получателей `ADMIN`, `SALES_MANAGER`, `FRANCHISEE`;
- настраивать группы событий `Продажи`, `Склад`, `Поставки`, `Администрирование`;
- задавать ручных получателей по `chat_id` и `@username`;
- видеть список “Недавние чаты”, которые появились после команды `/start`.

Важно:

- личные уведомления идут только по numeric `chat_id`;
- `FRANCHISEE` получает только partner-scoped события по своим заявкам и партиям;
- token хранится в зашифрованном виде и требует `TELEGRAM_TOKEN_ENCRYPTION_KEY`.

## 9.12 Страница клона

Позволяет менять тексты публичной страницы цифрового паспорта без правки кода.

Есть:

- live preview
- demo preview
- preview по реальному `serial_number`
- настройка `media_heading`, `media_empty_text`, `photo_button_text`, `video_button_text`

## 10. Работа франчайзи

## 10.1 Dashboard

Показывает:

- баланс;
- открытые задачи на сбор;
- задачи в работе;
- партии в `TRANSIT`;
- партии в `RECEIVED`;
- партии в `FINISHED`;
- последние партии.

## 10.2 Принятие задачи

Партнер видит доступные `CollectionRequest` и может взять задачу в работу.

Результат:

- `OPEN -> IN_PROGRESS`

## 10.3 Создание партии

Партнер:

1. выбирает задачу в работе;
2. заполняет координаты;
3. указывает дату и время сбора;
4. создает партию.

Система:

- создает batch в `TRANSIT`
- создает item
- генерирует `serial_number`

Если HQ создает задачу из каталога и в поле партнера выбирает `Принять сразу`, администратор указывает дату и время сбора. Система сразу создает связанную партию в `RECEIVED`; она появляется в `Товары → Партии` (`/admin/acceptance/batches`) и готова к загрузке фото и видео без подтверждения партнером.

## 10.4 My Batches

Партнер может:

- смотреть только свои партии;
- фильтровать по статусу;
- искать по `batch id`, `item id`, `temp_id`;
- выгружать текущую выборку в CSV;
- видеть количество проданных и непроданных позиций.

## 10.5 Finance

Показывает:

- текущий баланс;
- `commission_rate`;
- ledger;
- фильтры по периоду и типу операции;
- экспорт в CSV.

## 11. Публичный цифровой паспорт

### URL и API

- страница: `/clone/:serialNumber`
- данные: `GET /api/public/items/:serialNumber`
- QR PNG: `GET /api/public/items/:serialNumber/qr`

### Когда паспорт доступен

- batch в `RECEIVED` или `FINISHED`
- item не `REJECTED`
- `serial_number` не legacy

### Что видно в паспорте

- название товара;
- описание;
- дата и время сбора;
- координаты;
- фото;
- видео;
- серийный номер.

Источник media в API:

- фото возвращается как `item_photo_url`, а если его нет — как fallback из `photo_url`;
- видео возвращается из `item_video_url`.

## 12. Публичная активация

Endpoint:

- `POST /api/public/items/:serialNumber/activate`

Он:

- ставит `ACTIVATED`;
- пишет `activation_date`;
- отмечает item как `is_sold`.

Он не:

- создает бухгалтерскую проводку;
- меняет баланс франчайзи.

## 13. Файлы и медиа

Основные папки:

- `public/uploads/photos`
- `public/uploads/videos`
- `public/uploads/videos/generated`
- `public/uploads/videos/exports`
- `storage/uploads/staging`

Заметка по безопасности:
- `POST /api/upload/photo` принимает только raster-изображения (`png/jpg/gif/webp`), canonicalizes extension и блокирует HTML/SVG/XML payload.
- Для batch-нормализации shared photo upload backend использует ограниченную параллельность; `PHOTO_CONCURRENCY` управляет числом одновременно обрабатываемых фото.
- На `macOS arm64` HEIC/HEIF по умолчанию конвертируются через системный `sips`; `HEIC_APPLE_SILICON_CONCURRENCY` управляет отдельным лимитом параллельности для этого fast-path.
- `POST /api/upload/video` принимает только `mp4/mov/m4v/webm`; uploaded files под `/uploads/*` отдаются с `X-Content-Type-Options: nosniff`.
- `/admin/settings` содержит файловое окно `public/uploads`: просмотр размеров, загрузка файлов, создание папок и прямое удаление. Доступ только для `ADMIN`. Для UUID-папок партий доступен режим отображения `локация | шаблон | дата`, фильтр по локации и сортировка по дате партии.

Для baseline-замеров включите `VIDEO_PIPELINE_DIAGNOSTICS=1` и сравнивайте логи по полям `stage`, `duration_ms`, `active_jobs`, `active_tasks`, `active_renders`, `processed_count`, `total_count`, `size_bytes`, `job_concurrency`, `render_concurrency`, `ffmpeg_threads`, `converter`, `heic_concurrency`, `platform`, `arch`.
Ключевые stage: `probe`, `normalize`, `preview_render`, `final_render`, `final_concat`, `photo_convert`, `photo_batch`, `job`.

## 14. Частые проблемы

### 14.1 Не поднимается backend

Проверьте:

- заполнены ли `ACCESS_TOKEN_SECRET` и `REFRESH_TOKEN_SECRET`
- заполнены ли при интеграции доставки `CDEK_CLIENT_ID` и `CDEK_CLIENT_SECRET`
- доступна ли MySQL
- корректен ли `DATABASE_URL`

### 14.2 Не работает buyer login/register

Проверьте:

- используется ли `username` для buyer-регистрации
- не занят ли уже логин
- корректно ли сохранен `accessToken` в `localStorage`
- приходит ли `stones_refresh_token` как `HttpOnly` cookie на `/auth/login` и `/auth/refresh`

### 14.3 Photo Tool не сохраняет назначения

Проверьте:

- batch уже в `RECEIVED`
- у всех item есть `item_seq`
- каждая позиция партии покрыта уникальной фотографией
- страницу не успел пересохранить другой оператор

### 14.4 Video Tool не завершает экспорт

Проверьте:

- Video Tool открыт в HQ Desktop, не в браузере;
- исходники `READY`, товарных segment ровно столько же, сколько items;
- нет segment короче 500 ms и нет item без `serial_number`;
- при `AUTH_REQUIRED` выполните синхронизацию входа, при `PAUSED_OFFLINE` проверьте сеть;
- failed render/upload повторяется на уровне item или пачкой из Export.

### 14.5 Партия не финализируется

Проверьте:

- batch уже в `RECEIVED`
- нет активной video-задачи
- у каждого item есть фото и видео

### 14.6 QR не открывается

Проверьте:

- item не `REJECTED`
- batch в `RECEIVED` или `FINISHED`
- у item есть актуальный `serial_number`

## 15. Что не реализовано полностью

- реальная онлайн-оплата;
- Telegram login для buyer-аккаунта;
- автоматический финансовый расчет из публичной активации;
- новый UI-поток для офлайн-консигнации.
