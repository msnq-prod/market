...
`ADMIN` и `SALES_MANAGER` работают с заказами покупателей через четыре раздела:

- `Заказы` — очередь заявок, статусы, доставка и внутренние заметки;
- `Клиенты` — клиентская база и история заказов;
- `Наличие в продаже` — плотная таблица онлайн-остатков с поиском по названию, локации, кодам, `serial_number` и `temp_id`;
- `История продаж` — завершенные продажи и возвраты.

На странице `Наличие в продаже` доступны фильтры по локации, остатку, публикации и цене, сортировка по основным колонкам, 300 строк на странице по умолчанию и раскрытие товара до конкретных `Item`. Ручное резервирование, создание заказов и массовые операции на этой странице не выполняются.
...
- количество товаров и локаций в одной карточке;
- количество пользователей и франчайзи в одной карточке;
- партии в `TRANSIT`;
- партии в `RECEIVED`;
- item в `STOCK_HQ`;
- item в `STOCK_ONLINE`.
...
- из карточки партии доступны быстрые переходы в `Photo Tool`, `Video Tool` и `QR-печать`; в браузере media tools показывают заглушку со скачиванием `ZAGARAMI HQ`.
...
1. открыть сервис из `Приемки`, `Товаров и локаций` или из сайдбара `QR-печать` (из сайдбара сервис открывается в отдельном окне)
2. выбрать сохраненный пресет печати или настроить текущую конфигурацию вручную
3. выбрать товар-шаблон и партию
4. переключить режим `Все QR` или `Выбрать вручную`
5. настроить размеры этикетки, внутренние отступы, скругление углов, QR, шрифты и состав полей
6. при включенном поле `Свое поле` заполнить текст в списке позиций блока `Источник данных`
...
Если HQ создает задачу из каталога и в поле партнера выбирает `Принять сразу`, администратор указывает дату и время сбора. Система сразу создает связанную партию в `RECEIVED`; она появляется в `/admin/acceptance` и готова к загрузке фото и видео без подтверждения партнером.
