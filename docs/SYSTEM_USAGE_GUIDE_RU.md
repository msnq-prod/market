...
3. переключить режим `Все QR` или `Выбрать вручную`
4. настроить размеры этикетки, скругление углов, QR, шрифты и состав полей
5. при включенном поле `Свое поле` заполнить текст прямо в этикетках документа
6. сохранить документ через `Сохранить PDF`

По умолчанию этикетки прямоугольные (`Скругление, мм` = `0`). Перед сохранением сервис проверяет, что A4-страницы, размеры этикеток и QR соответствуют заданным параметрам.
