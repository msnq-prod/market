...
- `telegram-worker` — отдельный worker для Telegram outbox и `getUpdates`

### Особенности локального compose

- `app` запускается с `RUN_MIGRATIONS=1`
- `video-processor` запускается с `RUN_MIGRATIONS=0`
- `telegram-worker` запускается с `RUN_MIGRATIONS=0`
...
`app`, `db`, `video-processor` и `telegram-worker` остаются внутренними сервисами compose.
...
По умолчанию `preflight` требует минимум 2 GiB свободного места.
...
docker compose --env-file .env.production -f docker-compose.prod.yml logs --tail 100 telegram-worker
```
...
- `telegram-worker` — обязательная часть production стека, если включены Telegram-уведомления.
